    ...
    -- ─── Gitsigns ────────────────────────────────────────────────────────────
    {
        ...
        opts = {
            ...
            on_attach = function(bufnr)
                ...
                map("n", "]h", function() gs.nav_hunk("next") vim.schedule(gs.preview_hunk) end, { buffer = bufnr, desc = "Git: Next hunk" })
                map("n", "[h", function() gs.nav_hunk("prev") vim.schedule(gs.preview_hunk) end, { buffer = bufnr, desc = "Git: Prev hunk" })
                map("n", "]H", function() gs.nav_hunk("next", { target = "staged" }) vim.schedule(gs.preview_hunk) end, { buffer = bufnr, desc = "Git: Next staged hunk" })
                map("n", "[H", function() gs.nav_hunk("prev", { target = "staged" }) vim.schedule(gs.preview_hunk) end, { buffer = bufnr, desc = "Git: Prev staged hunk" })
